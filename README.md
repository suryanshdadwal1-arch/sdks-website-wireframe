...
The presentation shell keeps the supplied school name, “(To be affiliated to CBSE)” wording and aegis line. In parent view, the compact top strip links to Contact and Admission information; it does not invent phone, email or social destinations, and notes that official contact details are awaiting confirmation. Review view retains the full labelled missing-input strip, including social links. The header uses a nine-link desktop navigation plus a separate “Apply Now” CTA that routes to Admission information; the mobile menu preserves the CTA and internal page links. Applications are not submitted from this wireframe.

The current presentation consolidation also centralizes runtime content overrides in `website-content.js`, generated by `../consolidate-presentation.cjs`. The former override files remain as history and are not loaded by the consolidated presentation. The parent presentation currently covers 12 pages: Home, the nine navigation pages, Disclosure and Privacy; Fees is merged into Admission, Gallery into Infrastructure, and empty Leadership/Updates pages are removed.
